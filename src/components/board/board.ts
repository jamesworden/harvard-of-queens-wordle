...
class Board {
	...
	private getRow(rowIndex: number) {
		return this.self.getElementsByClassName(this.tileRowClassName).item(rowIndex);
	}
	...
	updateTileTextAndBorderColor(currentGuess: string, rowIndex: number) {
		for (let tileIndex = 0; tileIndex < this.getTilesFromRow(rowIndex).length; tileIndex++) {
